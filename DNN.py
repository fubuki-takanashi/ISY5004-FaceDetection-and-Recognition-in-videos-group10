...
model_dir = r'D:\Document\5004\video_store\models'
modelFile = os.path.join(model_dir, "res10_300x300_ssd_iter_140000.caffemodel")
configFile = os.path.join(model_dir, "deploy.prototxt.txt")

if not os.path.exists(modelFile):
    raise FileNotFoundError(f"{modelFile} not found.")
...
def process_image(filename):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            pred_boxes.append([startX, startY, endX, endY])
...
def compute_iou(box1, box2):
    ...
    inter_area = max(0, xi2 - xi1) * max(0, yi2 - yy1)
